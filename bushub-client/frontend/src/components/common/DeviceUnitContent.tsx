  ...
  return (
    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
      {groupedDevices.map((device: any) => (
        <Card
          key={device.deviceId}
          className='border-2 py-0 hover:border-primary/50 transition-colors min-w-0 border-gray-200 dark:border-gray-600 h-fit'
        >
          <div className='flex items-center gap-4 p-4'>
            <div
              className={`flex items-center justify-center w-16 h-16 rounded-lg flex-shrink-0 text-gray-900 dark:text-gray-100 ${
                smartcityMetaHelpers.getDeviceStyle(device.deviceId)?.avatarBg ??
                smartcityMetaHelpers.getDeviceStyle(device.deviceId)?.bgColor ??
            ...
            <div className='flex flex-col items-start min-w-0 flex-1'>
              <h3 className='font-semibold text-lg truncate text-black dark:text-gray-100'>{device.deviceName}</h3>
              <div className='flex flex-wrap gap-1 mt-1'>
                {device.units.map((unit: any) => (
                  <Badge key={unit.unitId} variant='secondary' className='text-xs border-gray-200 dark:border-gray-600'>
                    {unit.unitName}
                  </Badge>
                ))}
              </div>
