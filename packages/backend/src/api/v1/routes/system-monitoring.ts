...
async function getPollingStatus(): Promise<PollingStatus> {
  try {
    ...
    logDebug(`[getPollingStatus] 폴링 상태 조회 실패: ${error}`);
    return {
      enabled: false,
      interval: 20000,
...
async function getPollingRecoveryStatus(): Promise<PollingRecoveryStatus> {
  try {
    ...
    logDebug(`[getPollingRecoveryStatus] 폴링 복구 상태 조회 실패: ${error}`);
    return {
      isRunning: false,
...
async function getDdcTimeSyncStatus(): Promise<DdcTimeSyncStatus> {
  try {
    ...
    logDebug(`[getDdcTimeSyncStatus] DDC 시간 동기화 상태 조회 실패: ${error}`);
    return {
      isRunning: false,
      lastSyncTime: null,
      syncCount: 0,
...
async function checkExternalServer(url: string) {
  ...
  try {
    ...
    // 디버깅 로그
    logDebug(`[checkExternalServer] ExternalHttpClient 결과: ok=${result.ok}, status=${result.status}`);
...
async function systemMonitoringRoutes(app: FastifyInstance) {
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        try {
          ...
          logDebug(`[GET /system/monitoring] 서비스 도메인 상태 수집 실패: ${error}`);
          domainStatus = {
            deviceDomain: {},
            systemDomain: {},
            userDomain: {},
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        logDebug('[GET /database/collections] 데이터베이스 컬렉션 목록 요청');
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        logDebug(`[GET /database/collection/${name}] 컬렉션 데이터 요청 (페이지: ${pageNum}, 제한: ${limitNum})`);
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        logDebug(`[POST /system/external-check] 외부 서버 상태 확인 요청: ${url}`);
