...
function generateDeviceCommandSchema(deviceType: string) {
  try {
    ...
    if (!deviceCommands) {
      logDebug(`[generateDeviceCommandSchema] 디바이스 타입 '${deviceType}'에 대한 명령어가 없음`);
      return null;
    }
...
async function deviceRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // logDebug(`명령 조회 요청: ${deviceId}/${unitId}${ids ? ` (상태 조회: ${ids})` : ' (사용 가능한 명령 목록)'}`);
      ...
      try {
        ...
        if (ids) {
          ...
          // logDebug(`🔍 명령 상태 조회 요청: ${deviceId}/${unitId}, requestIds: ${JSON.stringify(requestIds)}`);
          ...
          // logDebug(`🔍 ObjectId 변환 결과: ${JSON.stringify(objectIds)}`);
          ...
          // logDebug(`🔍 CommandLog 조회 조건: ${JSON.stringify(queryCondition)}`);

          const logs = await CommandLog.find(queryCondition).lean();
          // logDebug(`🔍 CommandLog 조회 결과: ${JSON.stringify(logs)}`);
          ...
          // logDebug(`🔍 최종 응답 데이터: ${JSON.stringify(result)}`);
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      logDebug(`다중 명령 실행 요청: ${deviceId}/${unitId}, 명령 수: ${Array.isArray(commands) ? commands.length : 0}`);
      ...
      if (Array.isArray(commands) && commands.length > 0) {
        ...
        logDebug(`명령 상세 정보: ${JSON.stringify(commandDetails, null, 2)}`);
      }
      ...
      try {
        ...
        logDebug(`명령 등록 시작: ${deviceId}/${unitId}, 유닛 타입: ${unit.type}`);
        ...
        logDebug(`명령 등록 완료: ${deviceId}/${unitId}, 등록된 명령 수: ${commandLogs.length}`);
        logDebug(`🔍 CommandLog 생성 결과: ${JSON.stringify(commandLogs)}`);
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      logDebug(`디바이스 명령어 스키마 조회 요청: ${deviceId}/${unitId}`);
  ...
  fastify.get(
    ...
    { preHandler: [fastify.requireAuth] },
    async (request: FastifyRequest, reply: FastifyReply) => {
      logDebug('모든 디바이스 타입의 명령어 스키마 조회 요청');
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      logDebug(`특정 디바이스 명령어 스키마 조회 요청: ${deviceId}`);
  ...
  fastify.get(
    ...
    { preHandler: [fastify.requireAuth] },
    async (request: FastifyRequest, reply: FastifyReply) => {
      logDebug('Device 도메인 서비스 상태 조회 요청');
