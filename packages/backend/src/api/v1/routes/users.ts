...
async function usersRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        logDebug('[GET /api/v1/internal/users] 사용자 목록 조회 요청');
        ...
        logDebug('[GET /api/v1/internal/users] 응답 완료');
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        logDebug(`[POST /api/v1/internal/users] 사용자 생성 요청: ${username}`);
        ...
        try {
          ...
          logDebug(`[POST /api/v1/internal/users] API 키 생성 완료: ${username}`);
        } catch (apiKeyError) {
        ...
        if (role === 'ex-user') {
          logDebug(`[POST /api/v1/internal/users] External 사용자 생성 완료: ${username}, role: ${newUser.role}`);
        }

        logDebug(`[POST /api/v1/internal/users] 사용자 생성 완료: ${username}`);
  ...
  app.put(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        logDebug(`[PUT /api/v1/internal/users/${userId}] 사용자 수정 요청`);
        ...
        if (apiKey) {
          try {
            ...
            if (userApiKey) {
              ...
              logDebug(`[PUT /api/v1/internal/users/${userId}] API 키 업데이트 완료`);
            } else {
              // API 키가 없으면 새로 생성
              const apiKeyType =
              ...
              logDebug(`[PUT /api/v1/internal/users/${userId}] API 키 생성 완료`);
            }
          } catch (apiKeyError) {
        ...
        logDebug(`[PUT /api/v1/internal/users/${userId}] 사용자 수정 완료`);
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        logDebug(`[POST /api/v1/internal/users/${userId}/change-password] 비밀번호 변경 요청`);
        ...
        logDebug(`[POST /api/v1/internal/users/${userId}/change-password] 비밀번호 변경 완료`);

        return createSuccessResponse('비밀번호 변경 성공');
      } catch (error) {
        logDebug('[POST /api/v1/internal/users/change-password] 에러 발생');
  ...
  app.delete(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        logDebug(`[DELETE /api/v1/internal/users/${userId}] 사용자 삭제 요청`);
        ...
        try {
          ...
          logDebug(`[DELETE /api/v1/internal/users/${userId}] 관련 API 키 삭제 완료`);
        } catch (apiKeyError) {
        ...
        logDebug(`[DELETE /api/v1/internal/users/${userId}] 사용자 삭제 완료`);
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        logDebug('[GET /users/domain/status] User 도메인 서비스 상태 조회 요청');
