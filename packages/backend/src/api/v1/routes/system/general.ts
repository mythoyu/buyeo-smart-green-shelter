...
export default async function systemGeneralRoutes(app: FastifyInstance) {
  ...
  app.post(SYSTEM_GENERAL_ENDPOINTS.SYSTEM, { preHandler: [app.requireAuth] }, async (request, reply) => {
    try {
      ...
      logDebug(`[POST /system] action: ${action} 요청`);
          ...
          try {
            ...
            setTimeout(() => {
              try {
                ...
                logDebug(`백엔드 재기동 실행 중 오류: ${error}`);
            ...
            logDebug(`백엔드 재기동 요청 중 오류: ${error}`);
  ...
  app.get(
    ...
    async (request, reply) => {
      try {
        logDebug('[GET /system/polling/interval] 폴링 간격 조회 요청');

        const settings = await systemService.getSettings();
        const pollingInterval = normalizePollingCycleGap(settings?.runtime?.pollingInterval);
        const pollingIntervalText = formatPollingCycleGapText(pollingInterval);

        logDebug(`[GET /system/polling/interval] 폴링 간격 조회 완료: ${pollingIntervalText}`);
        ...
        logDebug('[GET /system/polling/interval] 에러 발생');
  ...
  app.post(
        ...
        body: {
          ...
          properties: {
            pollingInterval: {
              ...
              description: '폴링 사이클 종료 후 다음 사이클까지 대기(ms)',
            },
          },
        },
        response: {
    ...
    async (request, reply) => {
      try {
        ...
        logDebug(`[POST /system/polling/interval] 폴링 간격 설정 요청: ${pollingInterval}ms`);

        if (!isAllowedPollingCycleGap(pollingInterval)) {
          return handleHttpError(
            new HttpValidationError('폴링 간격은 500, 1000, 2000, 3000, 5000, 10000ms 중 하나여야 합니다.'),
            reply,
          );
        }
        ...
        logDebug(`[POST /system/polling/interval] 폴링 간격 설정 완료: ${pollingIntervalText}`);
        ...
        logDebug('[POST /system/polling/interval] 에러 발생');
  ...
  app.post(
    ...
    async (_request, reply) => {
      try {
        logDebug('[POST /system/auto-mode] 자동모드 활성화 요청');

        const result = await systemService.setAllUnitsToScheduleMode();

        logDebug('[POST /system/auto-mode] 자동모드 활성화 완료');
        ...
        logDebug('[POST /system/auto-mode] 에러 발생');
  ...
  // 전시장 모드 조회
  app.get('/system/exhibition-mode', { preHandler: [app.requireAuth] }, async (_request, reply) => {
    try {
      const settings = await systemService.getSettings();
      return reply.send(
        createSuccessResponse('전시장 모드 조회 성공', {
          exhibitionMode: !!settings?.runtime?.exhibitionMode,
        }),
      );
    } catch (error) {
      return handleHttpError(new HttpSystemError('전시장 모드 조회 중 오류'), reply);
    }
  });

  // 전시장 모드 설정
  app.post('/system/exhibition-mode', { preHandler: [app.requireAuth] }, async (request, reply) => {
    try {
      const { exhibitionMode } = request.body as { exhibitionMode: boolean };

      const current = await systemService.getSettings();
      ...
      const updated = await systemService.updateSettings({
        runtime: {
          pollingEnabled: !!currentRuntime.pollingEnabled,
      ...
      return reply.send(
        createSuccessResponse('전시장 모드 설정 성공', {
          exhibitionMode: !!updated?.runtime?.exhibitionMode,
        }),
      );
    } catch (error) {
      return handleHttpError(new HttpSystemError('전시장 모드 설정 중 오류'), reply);
    }
  });
  ...
  app.post(
    ...
    async (_request, reply) => {
      try {
        logDebug('[POST /system/manual-mode] 수동모드 활성화 요청');

        const result = await systemService.setAllUnitsToManualMode();

        logDebug('[POST /system/manual-mode] 수동모드 활성화 완료');
        ...
        logDebug('[POST /system/manual-mode] 에러 발생');
  ...
  app.get(SYSTEM_GENERAL_ENDPOINTS.SYSTEM_DOMAIN_STATUS, { preHandler: [app.requireAuth] }, async (request, reply) => {
    try {
      logDebug('[GET /system/domain/status] System 도메인 서비스 상태 조회 요청');
  ...
  app.post(
    ...
    async (request, reply) => {
      try {
        ...
        logDebug(`[POST /system/ping-test] Ping 테스트 요청: ${ip}`);
        ...
        try {
          ...
          logDebug(`[POST /system/ping-test] 실행 명령어: ${pingCommand}`);
          ...
          logDebug(`[POST /system/ping-test] Ping 테스트 완료: ${ip} - 성공: ${pingResult.success}`);
          ...
          logDebug(`[POST /system/ping-test] Ping 실행 실패: ${execError}`);
        ...
        logDebug('[POST /system/ping-test] 에러 발생');
