...
function validateAndParseRequest(request: FastifyRequest) {
  ...
  if (!clientId) {
    logDebug('External 클라이언트 요청: id가 제공되지 않았습니다.');
    throw new HttpValidationError(ErrorMessages.TEST_CLIENT_REQUIRED_ID);
  }

  logDebug(`클라이언트 요청: ${clientId}, initialize: ${initialize}`);
...
async function ensurePeopleCounterIfNeeded(clientId: string): Promise<void> {
  ...
  logDebug(`[동기화] ${clientId}에 피플카운터(d082) 누락 → DB 생성`);
...
async function initializeDdcSettings(clientId: string) {
  logDebug(`DDC 설정 초기화 시작: ${clientId}`);
  ...
  try {
    ...
    logDebug(`DDC 설정 초기화 완료: ${clientId}`);
    return true;
  } catch (error) {
    logDebug(`DDC 설정 초기화 실패: ${clientId}, 에러: ${error}`);
    return false;
  }
}
...
async function initializeSystemSettings(clientId: string) {
  logDebug(`시스템 설정 초기화 시작: ${clientId}`);
  ...
  try {
    ...
    logDebug(`시스템 설정 초기화 완료: ${clientId}`);
    return true;
  } catch (error) {
    logDebug(`시스템 설정 초기화 실패: ${clientId}, 에러: ${error}`);
    return false;
  }
}
...
async function resetDatabase(clientId: string, includeHardware: boolean) {
  ...
  logDebug(`기존 클라이언트 데이터 삭제 완료 (${includeHardware ? '하드웨어 초기화 포함' : '소프트웨어만'})`);
}

/**
 * 현장등록 후 DO/DI 포트 할당 동기화
 * - initialize: TS 기본값으로 전체 재설정 (resetClientFromTs)
 * - 그 외: DB에 없는 유닛만 TS 기준 insert (seedMissingFromTs)
 */
async function syncPortAssignmentsAfterRegistration(clientId: string, initialize: boolean): Promise<void> {
  try {
    const { PortMappingService } = await import('../../../core/services/PortMappingService');
    const portMapping = PortMappingService.getInstance();

    await portMapping.purgePortAssignmentsExcept(clientId);

    if (initialize) {
      const result = await portMapping.resetClientFromTs(clientId);
      logDebug(`현장등록 접점 포트 초기화: ${clientId}, DO=${result.doInserted}, DI=${result.diInserted}`);
    } else {
      const result = await portMapping.seedMissingFromTs(clientId);
      logDebug(`현장등록 접점 포트 시드(누락분만): ${clientId}, inserted=${result.inserted}, skipped=${result.skipped}`);
    }
  } catch (error) {
    logError(`현장등록 접점 포트 동기화 실패: ${clientId} - ${error}`);
    // 포트 동기화 실패해도 클라이언트 등록은 계속 진행
  }
}

// 클라이언트 생성 함수
async function createClient(clientId: string) {
  logDebug(`클라이언트 재설정 및 하드웨어 초기화: ${clientId}`);

  // 클라이언트 정보를 하드코딩된 기본값으로 생성
  const selectedClient = clients.find((c) => c.id === clientId);

  if (!selectedClient) {
    logDebug(`클라이언트 ID ${clientId}를 찾을 수 없습니다.`);
    throw new HttpValidationError(ErrorMessages.TEST_CLIENT_NOT_FOUND);
  }

  logDebug(`클라이언트 ID ${clientId}`);
...
async function createDevicesAndUnits(savedClient: any, clientUnitInfo: any, initialize: boolean) {
  ...
  logDebug(`클라이언트 유닛 정보 조회 완료`);

  if (!clientUnitInfo) {
    logDebug(`클라이언트 ${savedClient.id}의 유닛 정보를 찾을 수 없습니다.`);
    throw new HttpValidationError(ErrorMessages.TEST_CLIENT_NOT_FOUND);
  }

  // 자동 장비 생성
  logDebug(`자동 장비 생성 시작: ${Object.keys(clientUnitInfo).length}개 장비 타입`);

  // 각 장비 타입별로 DeviceSchema와 UnitSchema 자동 생성
  for (const [deviceType, units] of Object.entries(clientUnitInfo)) {
    try {
      logDebug(`장비 타입 ${deviceType} 처리 시작`);

      // deviceType으로 deviceId 찾기
      let deviceId: string | undefined;
      try {
        deviceId = deviceMappingHelpers.getDeviceIdByType(deviceType);
        logDebug(`장비 타입 ${deviceType} -> deviceId: ${deviceId}`);
      } catch (error) {
        logDebug(`장비 타입 ${deviceType}에서 deviceId 조회 중 에러: ${error}`);
        continue;
      }

      if (!deviceId) {
        logDebug(`장비 타입 ${deviceType}에 대한 deviceId를 찾을 수 없습니다.`);
        continue;
      }
      ...
      logDebug(`장비 자동 생성 완료: ${deviceId} (${deviceName}) - ${(units as any[]).length}개 유닛`);
    } catch (error) {
      logDebug(`장비 타입 ${deviceType} 처리 중 에러 발생: ${error}`);
      continue;
    }
  }
}
...
async function buildClientResponse(savedClient: any, initialize: boolean) {
  ...
  logDebug(`전체 Client 수: ${allClients.length}`);
  logDebug(`전체 Device 수: ${allDevices.length}`);
  logDebug(`전체 Unit 수: ${allUnits.length}`);
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        // 6.5 접점 포트 할당 동기화 (장비/유닛 생성 이후)
        await syncPortAssignmentsAfterRegistration(clientId, initialize);

        // 7. 하드웨어 초기화 (initialize가 true일 때만)
        if (initialize) {
          // DDC 하드웨어 초기화 실행
          logDebug(`DDC 하드웨어 초기화 시작: ${clientId}`);
          try {
            const serviceContainer = ServiceContainer.getInstance();
            const dataApplyService = serviceContainer.getDataApplyService();

            const ddcInitResult = await dataApplyService.initializeDDC(clientId);
            logDebug(`DDC 하드웨어 초기화 완료: ${clientId} - ${ddcInitResult.appliedDevices}개 포트 성공`);
          } catch (error) {
