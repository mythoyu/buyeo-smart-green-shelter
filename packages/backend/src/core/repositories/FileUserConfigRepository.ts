  ...
  loadConfig(): ConfigFile {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== 'production') {
        logDebug(`[DEBUG] Config file path: ${this.configPath}`);
        logDebug(`[DEBUG] Current directory: ${process.cwd()}`);
        logDebug(`[DEBUG] __dirname: ${__dirname}`);
      }
      ...
      if (process.env.NODE_ENV !== 'production') {
        logDebug(`[DEBUG] Config loaded successfully`);
      }
