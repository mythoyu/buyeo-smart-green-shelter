...
export class ServiceContainer {
  ...
  public initializeServices(): void {
    const containerLog = new Logger(ServiceContainer.name);

    if (this.initialized) {
      containerLog.debug('이미 초기화됨 - 중복 초기화 방지');
      return; // 🆕 중복 초기화 방지
    }

    containerLog.info('서비스 초기화 시작...');

    // 🆕 Phase 2: 초기화 시작 시간 기록
    this.initializationTime = new Date();

    const appLogger: ILogger = new Logger('app');
    const userLogger = new Logger(UserService.name);
    const clientLogger = new Logger(ClientService.name);
    const apiKeyLogger = new Logger(ApiKeyService.name);
    const modbusServiceLogger = new Logger(ModbusService.name);
    const userConfigLogger = new Logger(UserConfigService.name);
    const commandLogLogger = new Logger(CommandLogService.name);
    const unifiedLogLogger = new Logger(UnifiedLogService.name);
    const logSchedulerLogger = new Logger(LogSchedulerService.name);
    const securityLogger = new Logger(SecurityService.name);
    const linuxSystemLogger = new Logger(LinuxSystemService.name);
    const pollingRecoveryLogger = new Logger(PollingAutoRecoveryService.name);
    const controlLogger = new Logger(ControlService.name);
    const modbusPollerLogger = new Logger(UnifiedModbusPollerService.name);
    const dataSyncLogger = new Logger(DataSyncService.name);
    const errorLogger = new Logger(ErrorService.name);
    const systemLogger = new Logger(SystemService.name);
    const modbusQueueLogger = new Logger(ModbusCommandQueue.name);
    const modbusCommLogger = new Logger(UnifiedModbusCommunicationService.name);
    const pollingPersistenceLogger = new Logger(PollingDataPersistenceService.name);
    const unifiedModbusLogger = new Logger(UnifiedModbusService.name);
    const dataApplyLogger = new Logger(DataApplyService.name);
    const peopleCounterLogger = new Logger('PeopleCounter');

    this.services.set('logger', appLogger);
    ...
    const modbusCommandQueue = new ModbusCommandQueue(modbusQueueLogger);
    const unifiedModbusCommunicationService = new UnifiedModbusCommunicationService(modbusCommLogger);
    const pollingDataPersistenceService = new PollingDataPersistenceService(pollingPersistenceLogger);
    ...
    const unifiedModbusServiceInstance = new UnifiedModbusService(
      modbusCommandQueue,
      unifiedModbusCommunicationService,
      pollingDataPersistenceService,
      unifiedModbusLogger,
    );
    const unifiedModbusService: IUnifiedModbusCommunication = unifiedModbusServiceInstance;

    // 통합 Modbus 폴링 서비스 생성
    const unifiedModbusPollerService = new UnifiedModbusPollerService(unifiedModbusService, modbusPollerLogger);

    const dataSyncService = new DataSyncService(dataSyncLogger);
    ...
    const webSocketService: IWebSocketService = new WebSocketService(
      webSocketRepository,
      new Logger(WebSocketService.name),
    );
    const userService: IUserService = new UserService(userRepository, webSocketService, userLogger);
    const clientService: IClientService = new ClientService(clientRepository, webSocketService, clientLogger);
    const apiKeyService: IApiKeyService = new ApiKeyService(apiKeyRepository, webSocketService, apiKeyLogger);
    const modbusService: IModbusService = new ModbusService(
      modbusRepository,
      webSocketService,
      modbusServiceLogger,
      unifiedModbusServiceInstance,
    );
    ...
    const commandLogService: ICommandLogService = new CommandLogService(
      commandLogRepository,
      webSocketService,
      commandLogLogger,
    );
    const unifiedLogService: IUnifiedLogService = new UnifiedLogService(
      commandLogRepository,
      webSocketService,
      unifiedLogLogger,
    );
    ...
    const statusService: IStatusService = new StatusService(statusRepository, new Logger(StatusService.name));
    const errorService: IErrorService = new ErrorService(errorRepository, errorLogger);
    ...
    const systemService: ISystemService = new SystemService(systemRepository, webSocketService, systemLogger);

    const securityService: ISecurityService = new SecurityService(securityLogger);

    const linuxSystemService: ILinuxSystemService = new LinuxSystemService(linuxSystemLogger, securityService);
    ...
    // DataApplyService 생성
    const dataApplyService = new DataApplyService(dataApplyLogger);
    ...
    // 🔄 폴링 자동 복구 서비스 등록
    const pollingAutoRecoveryService = new PollingAutoRecoveryService(pollingRecoveryLogger);
    ...
    ports.forEach((port, idx) => {
      ...
      peopleCounterQueueServices.set(unitId, new PeopleCounterQueueService(peopleCounterLogger, port));
    });

    // 기본 서비스 키: 기존 코드 호환을 위해 u001을 peopleCounterQueueService로 노출
    const defaultQueue =
      peopleCounterQueueServices.get('u001') ??
      new PeopleCounterQueueService(peopleCounterLogger, ports[0] ?? PEOPLE_COUNTER_DISABLED_PATH);
    ...
    // 피플카운터 폴러 (APC100)
    const peopleCounterPoller = new PeopleCounterPollerService(peopleCounterLogger);
    ...
    containerLog.info('서비스 초기화 완료');
    containerLog.debug(`초기화 통계: ${this.services.size}개 서비스, ${initializationDuration}ms`);
    containerLog.debug(
      `도메인별 서비스: Device(${Object.keys(this.getDeviceDomainServices()).length}), System(${
