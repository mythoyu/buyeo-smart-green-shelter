...
import {
  ...
  GlobalBulkUnitPollingResult,
  RegisteredDeviceRef,
  unitResultKey,
} from '../../shared/utils/modbusCycleGlobalBulk';
import {
  createModbusBulkReadExecutor,
...
import {
  clearBulkPollingFailed,
  clearPollingSkippedNoClientId,
  notifyBulkPollingFailed,
  notifyModbusLink,
  notifyPollingCycleFailed,
  notifyPollingSkippedNoClientId,
  notifyUnitDdcCommunication,
} from './criticalWsLog';
import {
  applyMockUnitCommProbeFailure,
  clonePollingOutcome,
  getMockSimulateMode,
  isMockPollingFailureSimEnabled,
} from './mockPollingFailureSim';
...
export class UnifiedModbusPollerService {
  ...
  private currentCycleGapMs = DEFAULT_POLLING_CYCLE_GAP_MS;
  private nextPollingInterval: number | null = null; // 다음 사이클 종료 후 적용할 대기 간격
  private mappedPollId: string | null = null;
  ...
  /** deviceId → clientId (사이클당 1회 조회) */
  private clientIdCache = new Map<string, string>();

  /** USB/RS485 링크 감시 (폴링 사이클과 분리) */
  private static readonly LINK_MONITOR_INTERVAL_MS = 1500;
  private linkMonitorTimer: NodeJS.Timeout | null = null;
  private linkDownActive = false;
  /** Mock MODBUS_MOCK_SIMULATE_MODE=link_down */
  private mockLinkDownSimActive = false;
  ...
  constructor(modbusService: IUnifiedModbusCommunication, logger?: ILogger, onDataCallback?: (data: any) => void) {
    ...
    try {
      ...
      this.logger?.debug(`DataSyncService 초기화 완료`);
    } catch (error) {
      this.logger?.warn(`DataSyncService 초기화 실패: ${error}`);
      this.dataSyncService = null;
    }

    this.logger?.debug(`초기화 완료 (Mock 모드: ${this.mockMode})`);
  ...
  public setDataSyncService(dataSyncService: DataSyncService): void {
    ...
    this.logger?.debug(`DataSyncService 설정 완료`);
  }
  ...
  private startMemoryCleanupTimer(): void {
    ...
    this.logger?.info(`메모리 정리 타이머 시작 (${this.MEMORY_CLEANUP_INTERVAL}ms)`);
  }
  ...
  private performMemoryCleanup(): void {
    try {
      ...
      if (now - this.lastDeviceCacheUpdate > this.DEVICE_CACHE_TTL) {
        ...
        cleanedItems++;
        this.logger?.debug(`만료된 디바이스 캐시 정리`);
      }

      // 🎯 성능 메트릭 업데이트
      this.performanceMetrics.lastCleanup = new Date();
      this.logger?.debug(
        `메모리 정리 완료: ${cleanedItems}개 항목, 소요시간: ${Date.now() - startTime}ms`,
      );
    } catch (error) {
  ...
  private async getPollingInterval(): Promise<number> {
    try {
      ...
      return normalizePollingCycleGap(systemSettings?.runtime?.pollingInterval);
    } catch (error) {
      this.logger?.warn(
        `폴링 사이클 간격 설정을 가져올 수 없습니다. 기본값 사용: ${DEFAULT_POLLING_CYCLE_GAP_MS}ms`,
      );
  ...
  private async getSystemMode(): Promise<string> {
    try {
      ...
      this.logger?.error(`시스템 모드 확인 실패: ${error}`);
      return 'manual';
    }
  }
  ...
  > {
    try {
      ...
      if (this.deviceCache.length > 0 && now - this.lastDeviceCacheUpdate < this.DEVICE_CACHE_TTL) {
        this.logger?.debug(`캐시된 장비 목록 사용 (${this.deviceCache.length}개)`);
      ...
      if (!devices || devices.length === 0) {
        this.logger?.info(`Data 컬렉션이 비어있습니다.`);
        this.deviceCache = [];
      ...
      this.logger?.info(`등록된 장비 ${deviceList.length}개 발견`);
      return deviceList;
    } catch (error) {
      this.logger?.error(`등록된 장비 목록 조회 실패: ${error}`);
      return [];
    }
  }
  ...
  async schedulePollingIntervalChange(newInterval: number): Promise<void> {
    const gap = normalizePollingCycleGap(newInterval);
    this.nextPollingInterval = gap;
    this.logger?.info(`다음 폴링 사이클 종료 후 대기 간격을 ${gap}ms로 변경합니다.`);
  }
  ...
  async startBasicPolling(slaveId = 1, onData?: (data: any) => void): Promise<string> {
    try {
      // 🎯 3단계: 통합 폴링 시스템 사용
      this.logger?.info(`기본 폴링 시작 - Slave: ${slaveId}`);

      // �� 필수 폴링 시작 (시스템 상태만 확인)
      const pollIds = await this.startEssentialPolling(slaveId, DEFAULT_POLLING_CYCLE_GAP_MS, onData);

      if (pollIds.length > 0) {
        this.logger?.info(`기본 폴링 시작 완료 - ID: ${pollIds[0]}`);
        return pollIds[0];
      }
      throw new Error('기본 폴링 시작 실패');
    } catch (error) {
      this.logger?.error(`기본 폴링 시작 실패: ${error}`);
      throw error;
    }
  }
  ...
  stopBasicPolling(pollId: string): boolean {
    ...
    if (intervalId) {
      ...
      this.logger?.info(`기본 폴링 중지 - ID: ${pollId}`);
      return true;
    }
    return false;
  }
  ...
  async startSNGILUnifiedPolling(slaveId = 1, interval = 5000, onData?: (data: any) => void): Promise<string> {
    try {
      // 🎯 3단계: 통합 폴링 시스템 사용
      this.logger?.info(
        `SNGIL DDC 통합 폴링 시작 - Slave: ${slaveId}, 간격: ${interval}ms`,
      );
      ...
      if (pollIds.length > 0) {
        this.logger?.info(`통합 폴링 시작 완료 - ID: ${pollIds[0]}`);
        return pollIds[0];
      }
      throw new Error('폴링 시작 실패');
    } catch (error) {
      this.logger?.error(`SNGIL DDC 통합 폴링 시작 실패: ${error}`);
      throw error;
    }
  }
  ...
  stopPolling(pollId: string): boolean {
    ...
    if (intervalId) {
      clearTimeout(intervalId);
      this.pollingIntervals.delete(pollId);
      this.logger?.info(`폴링 중지 - ID: ${pollId}`);
      return true;
    }
    return false;
  }
  ...
  stopAllPolling(): void {
    ...
    this.stopLinkMonitor();
    // 통합 폴링 타이머 정리
    if (this.singlePollingTimer) {
      clearTimeout(this.singlePollingTimer);
    ...
    for (const [pollId, intervalId] of this.pollingIntervals.entries()) {
      clearTimeout(intervalId);
      this.logger?.debug(`폴링 중지 - ID: ${pollId}`);
    }
  ...
  async startAllSNGILPolling(slaveId = 1, onData?: (data: any) => void): Promise<string[]> {
    try {
      ...
      if (!(await this.isSystemModeAuto())) {
        this.logger?.warn('시스템 모드가 auto가 아닙니다. 폴링을 시작하지 않습니다.');
        return [];
      }

      // 🎯 통합 폴링 시스템 사용
      this.logger?.info(`통합 폴링 시스템으로 시작합니다.`);
      try {
        const serviceContainer = ServiceContainer.getInstance();
        ...
        if (queueStatus) {
          this.logger?.info(
            `현재 Modbus 큐 상태 - 총 ${queueStatus.totalCommands}개 (high: ${queueStatus.highPriority}, normal: ${queueStatus.normalPriority}, low: ${queueStatus.lowPriority})`,
          );
        }
      } catch (queueError) {
        this.logger?.warn(`큐 상태 조회 실패: ${queueError}`);
      }
      return this.startUnifiedPolling(slaveId, onData);
    } catch (error) {
      this.logger?.error(`통합 폴링 시작 실패: ${error}`);
      // 🎯 Fallback 제거 - 에러 발생 시 빈 배열 반환
      this.logger?.warn(`에러 발생으로 폴링을 시작하지 않습니다.`);
      return [];
    }
  }
  ...
  private clearClientIdCache(): void {
    this.clientIdCache.clear();
  }

  private async getCachedClientId(deviceId: string): Promise<string | null> {
    const cached = this.clientIdCache.get(deviceId);
    if (cached) {
      return cached;
    }
    ...
    if (clientId) {
      this.clientIdCache.set(deviceId, clientId);
    }
    return clientId;
  }

  private isCommonSystemDeviceType(deviceType: string): boolean {
    return deviceType === 'ddc_time' || deviceType === 'seasonal';
  }

  private createPollingModbusReadExecutor(contextLabel: string): ModbusReadCommandExecutor {
    return async ({ functionCode, address, length, commandId }) => {
      const modbusCommand = {
        id: `polling_${contextLabel}_${commandId}`,
        type: 'read' as const,
        unitId: '1',
        functionCode,
        address,
      ...
      return {
        ...
        data: Array.isArray(modbusResult?.data)
          ? modbusResult.data.map((v: boolean | number) => (typeof v === 'boolean' ? (v ? 1 : 0) : Number(v)))
          : undefined,
  ...
  /**
   * GET_* 액션을 bulk read로 실행 (TIME_INTEGRATED는 폴링 제외 — HOUR/MINUTE GET만)
   */
  private async executePollingActionsWithBulk(
    mapping: Record<string, unknown>,
    contextLabel: string,
  ): Promise<{ results: PollingActionResult[]; transactionCount: number }> {
    const bulkActions = getPollableGetActionNames(mapping);

    if (bulkActions.length === 0) {
      return { results: [], transactionCount: 0 };
    }

    const bulk = await executePollingBulkReads(
      mapping,
      bulkActions,
      this.createPollingModbusReadExecutor(contextLabel),
      {
        onGroupFailure: (groupKey, error) => {
          this.logger?.warn(
            `bulk read group failed (${contextLabel}): ${groupKey} - ${error}`,
          );
        },
      },
    );

    return { results: bulk.results, transactionCount: bulk.transactionCount };
  }
  ...
  async executePollingForDevice(deviceId: string, unitId: string, deviceType: string): Promise<any> {
    ...
    try {
      ...
      const clientMapping = getEffectiveClientMapping(clientId);
      ...
      const pollingActions = getPollableGetActionNames(unitMapping);

      if (pollingActions.length === 0) {
        this.logger?.warn(`No polling actions found for ${deviceId}/${unitId}`);
        await this.applyUnitCommunicationStatusFromPolling(deviceId, unitId, deviceType, {
          success: false,
        });
        return { success: false, message: 'No polling actions available' };
      }

      const contextLabel = `${deviceId}_${unitId}_${deviceType}`;
      this.logger?.debug(
        `Per-device bulk ${deviceId}/${unitId}: ${pollingActions.length} pollable action(s)`,
      );

      const bulkPoll = await this.executePollingActionsWithBulk(unitMapping, contextLabel);
      const results = bulkPoll.results;
      const transactionCount = bulkPoll.transactionCount;

      this.logger?.debug(
        `${deviceId}/${unitId}: ${transactionCount} Modbus transaction(s) (per-device bulk)`,
      );
      ...
      this.logger?.debug(
        `Polling completed: ${successCount}/${pollingActions.length} actions successful (${pollingStats.successRate}%), Response time: ${responseTime}ms`,
      );

      // 🆕 상세 성공/실패 로깅
      if (successCount > 0) {
        const successfulActions = results.filter((r) => r.success).map((r) => r.action);
        this.logger?.debug(`Successful actions: ${successfulActions.join(', ')}`);
      }

      if (results.some((r) => !r.success)) {
        const failedActions = results.filter((r) => !r.success).map((r) => `${r.action} (${r.error})`);
        this.logger?.warn(`Failed actions: ${failedActions.join(', ')}`);
      }
      ...
      this.logger?.error(
        `Polling failed for ${deviceId}/${unitId}: ${
  ...
  private async executeCommonSystemPortPolling(clientId: string, deviceType: string): Promise<any> {
    ...
    try {
      ...
      if (pollingActions.length === 0) {
        this.logger?.warn(`No polling actions found for ${deviceType}`);
        return { success: false, message: 'No polling actions available' };
      }

      this.logger?.info(
        `Starting COMMON_SYSTEM_PORTS polling for ${deviceType} with ${pollingActions.length} actions (bulk read)`,
      );

      const deviceTypeMapping = clientMapping[deviceType] as Record<string, unknown>;
      const { results, transactionCount } = await this.executePollingActionsWithBulk(
        deviceTypeMapping,
        `common_${clientId}_${deviceType}`,
      );

      this.logger?.debug(
        `COMMON_SYSTEM_PORTS bulk ${deviceType}: ${transactionCount} Modbus transaction(s)`,
      );
      ...
      this.logger?.info(
        `COMMON_SYSTEM_PORTS polling completed: ${successCount}/${pollingActions.length} actions successful, Response time: ${responseTime}ms`,
      );
      ...
      this.logger?.error(
        `COMMON_SYSTEM_PORTS polling failed for ${deviceType}: ${
          error instanceof Error ? error.message : String(error)
        }, Response time: ${responseTime}ms`,
      );
      return { success: false, error: error instanceof Error ? error.message : String(error), responseTime };
  ...
  private async executeCommonSystemPortActionWithRetry(
    ...
    for (let attempt = 1; attempt <= this.MAX_RETRY_ATTEMPTS; attempt++) {
      try {
        ...
        if (attempt > 1) {
          this.logger?.info(
            `COMMON_SYSTEM_PORTS action ${action} succeeded on attempt ${attempt} for ${deviceType}`,
          );
        }
        ...
        if (attempt < this.MAX_RETRY_ATTEMPTS) {
          ...
          this.logger?.warn(
            `COMMON_SYSTEM_PORTS action ${action} failed on attempt ${attempt} for ${deviceType}, retrying in ${delay}ms: ${lastError.message}`,
          );
  ...
  private async executePollingActionWithRetry(
    ...
    for (let attempt = 1; attempt <= this.MAX_RETRY_ATTEMPTS; attempt++) {
      try {
        ...
        if (attempt > 1) {
          this.logger?.info(
            `Action ${action} succeeded on attempt ${attempt} for ${deviceId}/${unitId}`,
          );
        }
        ...
        this.logger?.warn(
          `Action ${action} failed on attempt ${attempt}/${this.MAX_RETRY_ATTEMPTS} for ${deviceId}/${unitId}: ${lastError.message}`,
        );
  ...
  private async getClientIdFromDeviceCollection(deviceId: string): Promise<string | null> {
    try {
      ...
      if (device && device.clientId) {
        this.logger?.debug(`Device ${deviceId}의 clientId: ${device.clientId}`);
        return device.clientId;
      }

      this.logger?.warn(`Device ${deviceId}에서 clientId를 찾을 수 없음`);
      return null;
    } catch (error) {
      this.logger?.error(`Device 조회 실패: ${error}`);
      return null;
    }
  }
  ...
  private async executePollingAction(
    ...
    try {
      ...
      const clientId = await this.getClientIdFromDeviceCollection(deviceId);
      if (!clientId) {
        throw new Error(`Polling skipped: no clientId for device ${deviceId}`);
      }
      const clientMapping = getEffectiveClientMapping(clientId);
      ...
      if (foundAction === 'TIME_INTEGRATED') {
        // this.logger?.info(
        //   `🎯 통합시간 명령어 처리 시작: ${clientId}/${deviceType}/${unitId}/${action}`,
        // );
      ...
      this.logger?.debug(
        `📡 Modbus 커맨드: ${action} (FC:${foundAction.port.functionCode}, Addr:${foundAction.port.address}) - ${commandDuration}ms`,
      );
  ...
  private async executeIntegratedTimeAction(
    ...
    try {
      // this.logger?.info(
      //   `통합된 시간 명령어 실행: ${clientId}/${deviceType}/${unitId}/${action}`,
      // );
      ...
      // 성능 로그 (DEBUG 레벨)
      // this.logger?.debug(
      //   `📡 통합시간 명령어: ${action} - HOUR ${hourDuration}ms | MINUTE ${minuteDuration}ms`,
      // );

      // 🎯 HOUR/MINUTE 결과를 통합된 "HH:mm" 형식으로 조합
      const integratedTime = this.combineHourMinute(hourResult, minuteResult);

      // this.logger?.info(`통합된 시간 명령어 완료: ${action} = ${integratedTime}`);
  ...
  getPollingStats(): Record<string, any> {
    return {
      ...
      defaultPollingInterval: this.currentCycleGapMs,
  ...
  destroy(): void {
    try {
      ...
      if (this.memoryCleanupTimer) {
        ...
        this.logger?.info(`메모리 정리 타이머 정리 완료`);
      }

      // 🎯 캐시 정리
      this.deviceCache = [];
      this.lastDeviceCacheUpdate = 0;
      this.clearClientIdCache();
      ...
      this.logger?.info(`서비스 정리 완료 (메모리 정리 포함)`);
    } catch (error) {
      this.logger?.error(`서비스 정리 중 오류 발생: ${error}`);
    }
  }
   ...
   * 새로운 폴링 (매핑 방식)
   */
  private async startMappedPolling(_slaveId: number, _onData?: (data: any) => void): Promise<string[]> {
    try {
      const cycleGapMs = await this.getPollingInterval();
      const pollId = this.startMappedPollingWithTimer(cycleGapMs);

      this.logger?.info(`매핑 기반 폴링 시작 완료 - ID: ${pollId}`);
      return [pollId];
    } catch (error) {
      this.logger?.error(
        `매핑 기반 폴링 시작 실패: ${
  ...
  private clearMappedPollingTimer(): void {
    if (this.singlePollingTimer) {
      clearTimeout(this.singlePollingTimer);
      this.singlePollingTimer = null;
    }
  ...
  private resolveCycleGapAfterCycle(): number {
    if (this.nextPollingInterval !== null) {
      this.currentCycleGapMs = normalizePollingCycleGap(this.nextPollingInterval);
      this.nextPollingInterval = null;
      this.logger?.info(`폴링 사이클 대기 간격 적용: ${this.currentCycleGapMs}ms`);
    }
    return this.currentCycleGapMs;
  }

  private scheduleNextMappedPollingCycle(gapMs: number): void {
    if (this.stopRequested) {
      return;
    }
    this.clearMappedPollingTimer();
    this.currentCycleGapMs = gapMs;
    this.singlePollingTimer = setTimeout(() => {
      void this.runMappedPollingCycle();
    }, gapMs);
    if (this.mappedPollId) {
      this.pollingIntervals.set(this.mappedPollId, this.singlePollingTimer);
    }
  }

  /** 폴링 1사이클 실행 후, 종료 시점부터 cycleGapMs 만큼 대기하고 다음 사이클 예약 */
  private async runMappedPollingCycle(): Promise<void> {
    if (this.stopRequested) {
      return;
    }
    if (this.isPollingCycleRunning) {
      this.logger?.warn(
        '이전 폴링 사이클이 아직 실행 중입니다. 다음 대기 후 재시도합니다.',
      );
      this.scheduleNextMappedPollingCycle(this.currentCycleGapMs);
      return;
    }
    ...
    try {
      ...
      if (currentDevices.length === 0) {
        this.logger?.info(`Data 컬렉션이 비어있습니다. 이번 주기 폴링을 건너뜁니다.`);
      } else {
        await this.executeMappedPollingForAllDevices();
      }
    } catch (error) {
      this.logger?.error(
        `매핑 기반 폴링 실패: ${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      this.isPollingCycleRunning = false;
      const gapMs = this.resolveCycleGapAfterCycle();
      this.scheduleNextMappedPollingCycle(gapMs);
    }
  }

  /**
   * 매핑 기반 폴링 루프 시작 (첫 사이클 즉시 실행 → 종료 후 대기 → 반복)
   */
  private startMappedPollingWithTimer(cycleGapMs: number): string {
    const pollId = `mapped_polling_${Date.now()}`;
    this.stopRequested = false;
    this.clearMappedPollingTimer();
    this.currentCycleGapMs = normalizePollingCycleGap(cycleGapMs);
    this.mappedPollId = pollId;
    ...
    this.logger?.info(
      `매핑 기반 폴링 시작 - ID: ${pollId}, 사이클 종료 후 대기: ${this.currentCycleGapMs}ms`,
    );
  ...
  private async executeMappedPollingForAllDevices(): Promise<void> {
    ...
    try {
      this.logger?.debug(`🚀 폴링 사이클 시작 - ${new Date().toISOString()}`);

      if (this.stopRequested) {
        this.logger?.debug('폴링 중지 요청 감지 - 사이클 실행을 건너뜁니다.');
        return;
      }

      this.clearClientIdCache();

      if (this.mockMode && isMockPollingFailureSimEnabled() && getMockSimulateMode() === 'link_down') {
        if (!this.mockLinkDownSimActive) {
          this.mockLinkDownSimActive = true;
          this.linkDownActive = true;
          const devices = await this.getRegisteredDevicesWithCache();
          await this.markDevicesCommunicationError(devices);
          notifyModbusLink(true);
          this.logger?.warn('[UnifiedModbusPoller] Mock link_down 시뮬 — 사이클 스킵');
        }
        return;
      }
      this.mockLinkDownSimActive = false;
      ...
      try {
        ...
        this.logger?.warn(`웹소켓 로그 전송 실패: ${wsError}`);
      }
      ...
      const batchForSync: Array<{
        deviceId: string;
        unitId: string;
        deviceType: string;
        pollingResult: unknown;
        timestamp: Date;
      }> = [];
      let totalGlobalModbusTransactions = 0;

      const devicesByClient = new Map<string, RegisteredDeviceRef[]>();
      for (const device of devices) {
        const clientId = await this.getCachedClientId(device.deviceId);
        if (!clientId) {
          this.logger?.warn(`폴링 제외: device ${device.deviceId}에 clientId 없음`);
          notifyPollingSkippedNoClientId(device.deviceId);
          continue;
        }
        clearPollingSkippedNoClientId(device.deviceId);
        const list = devicesByClient.get(clientId) ?? [];
        list.push(device);
        devicesByClient.set(clientId, list);
      }

      for (const [clientId, clientDevices] of devicesByClient.entries()) {
        if (this.stopRequested) {
          this.logger?.debug('폴링 중지 요청으로 남은 장비 처리를 중단합니다.');
          break;
        }

        const regularDevices = clientDevices.filter((d) => !this.isCommonSystemDeviceType(d.deviceType));

        const globalResults = new Map<string, GlobalBulkUnitPollingResult>();

        if (regularDevices.length > 0) {
          const globalBulkStart = Date.now();
          try {
            if (this.mockMode && isMockPollingFailureSimEnabled() && getMockSimulateMode() === 'bulk_fail') {
              throw new Error('Mock bulk polling failure simulation');
            }
            const globalBulk = await executeGlobalBulkPollingForClient(
              clientId,
              regularDevices,
              createModbusBulkReadExecutor(this.createPollingModbusReadExecutor(`global_${clientId}`)),
            );
            totalGlobalModbusTransactions += globalBulk.transactionCount;

            this.logger?.debug(
              `Global bulk polling (${clientId}): ${regularDevices.length} devices, ${
                globalBulk.groupCount
              } groups, ${globalBulk.transactionCount} transactions, ${Date.now() - globalBulkStart}ms`,
            );

            for (const [key, unitResult] of globalBulk.unitResults.entries()) {
              globalResults.set(key, unitResult);
            }
            clearBulkPollingFailed(clientId);
          } catch (bulkError) {
            const bulkDetail =
              bulkError instanceof Error ? bulkError.message : String(bulkError);
            this.logger?.error(`Global bulk polling failed (${clientId}): ${bulkDetail}`);
            notifyBulkPollingFailed(clientId, bulkDetail);
            await this.markDevicesCommunicationError(regularDevices);
          }
        }
        ...
        for (const device of clientDevices) {
          ...
          try {
            let pollingResult: GlobalBulkUnitPollingResult | Record<string, unknown> | undefined;

            if (this.isCommonSystemDeviceType(device.deviceType)) {
              pollingResult = await this.executeCommonSystemPortPolling(
                (await this.getCachedClientId(device.deviceId)) || clientId,
                device.deviceType,
              );
            } else {
            ...
            this.logger?.debug(
              `✅ 장비 폴링 완료: ${device.deviceId}/${device.unitId} - ${deviceDuration}ms`,
            );
          } catch (error) {
            const deviceDuration = Date.now() - deviceStartTime;
            this.logger?.error(
              `❌ 장비 폴링 실패: ${device.deviceId}/${
                device.unitId
              } - ${deviceDuration}ms 후 실패: ${error instanceof Error ? error.message : String(error)}`,
            );
            await this.applyUnitCommunicationStatusFromPolling(device.deviceId, device.unitId, device.deviceType, {
              success: false,
            });
          }
        }
      ...
      if (batchForSync.length > 0 && this.dataSyncService) {
        await this.dataSyncService.syncBatchPollingData(batchForSync);
      }
      const dataSyncBatchMs = Date.now() - dataSyncStartTime;

      this.logger?.debug(
        `사이클 Modbus: global ${totalGlobalModbusTransactions} transaction(s) | DataSync 일괄 ${dataSyncBatchMs}ms (${batchForSync.length}개 유닛)`,
      );

      if (this.stopRequested) {
        this.logger?.debug('폴링 중지 요청으로 사이클을 조기에 종료했습니다.');
        return;
      }
      ...
      // 상세 성능 분석 로그
      this.logger?.debug(
        `✅ 폴링 사이클 완료 - 총 ${cycleDuration}ms (${devices.length}개 장비)`,
      );
      this.logger?.debug(
        `📊 성능 분석: 장비조회 ${deviceQueryTime}ms | 폴링조립 ${totalPollingTime}ms | DataSync일괄 ${dataSyncBatchMs}ms`,
      );
      ...
      const cycleEndTime = Date.now();
      const cycleDuration = cycleEndTime - cycleStartTime;
      this.logger?.error(`❌ 폴링 사이클 실패 - ${cycleDuration}ms 후 실패: ${error}`);

      const errDetail = error instanceof Error ? error.message : String(error);
      notifyPollingCycleFailed(cycleDuration, errDetail);
    }
  }
  ...
  private async startUnifiedPolling(slaveId: number, onData?: (data: any) => void): Promise<string[]> {
    try {
      this.logger?.info(`MAPPED_ONLY 폴링 시작`);

      // 🎯 MAPPED_ONLY만 실행
      return this.startMappedPolling(slaveId, onData);
    } catch (error) {
      this.logger?.error(
        `MAPPED_ONLY 폴링 시작 실패: ${
          error instanceof Error ? error.message : String(error)
        }`,
      );
      // 🎯 Fallback 제거 - 에러 발생 시 빈 배열 반환
  ...
  private async startEssentialPolling(
    ...
    try {
      ...
      this.logger?.info(`필수 폴링 시작 - ID: ${pollId}, 간격: ${interval}ms`);
      return [pollId];
    } catch (error) {
      this.logger?.error(
        `필수 폴링 시작 실패: ${error instanceof Error ? error.message : String(error)}`,
      );
      return [];
    }
  }
  ...
  private async executeEssentialPolling(slaveId: number): Promise<void> {
    try {
      ...
      this.logger?.debug(`필수 폴링 실행 - 시스템 상태 확인`);
      ...
      if (systemStatusResult.success) {
        this.logger?.debug(`시스템 상태 확인 성공 - Slave: ${slaveId}`);
        ...
        this.logger?.warn(`시스템 상태 확인 실패 - Slave: ${slaveId}`);
      }
    } catch (error) {
      this.logger?.error(
        `필수 폴링 실행 실패: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        if (!this.dataSyncService) {
          this.logger?.warn(`DataSyncService가 초기화되지 않았습니다.`);
          return false;
        }
        ...
        if (syncResult.success) {
          this.logger?.debug(
            `폴링 데이터 저장 성공: ${deviceId}/${unitId} - ${syncResult.updatedFields}개 필드`,
          );
          return true;
        }
        ...
        if (attempt === maxRetries) {
          this.logger?.error(
            `폴링 데이터 저장 실패 (${maxRetries}회 시도): ${deviceId}/${unitId} - ${errorMessage}`,
          );
          return false;
        }
        this.logger?.warn(
          `폴링 데이터 저장 재시도 (${attempt}/${maxRetries}): ${deviceId}/${unitId} - ${errorMessage}`,
        );
        // 재시도 전 지연 (지수 백오프)
  ...
  private startLinkMonitor(): void {
    this.stopLinkMonitor();
    if (this.mockMode) {
      return;
    }

    this.linkMonitorTimer = setInterval(() => {
      void this.runLinkMonitorTick();
    }, UnifiedModbusPollerService.LINK_MONITOR_INTERVAL_MS);

    this.logger?.info(
      `Modbus 링크 감시 시작 (${UnifiedModbusPollerService.LINK_MONITOR_INTERVAL_MS}ms)`,
    );
  }

  private stopLinkMonitor(): void {
    if (this.linkMonitorTimer) {
      clearInterval(this.linkMonitorTimer);
      this.linkMonitorTimer = null;
    }
    this.linkDownActive = false;
  }

  private async runLinkMonitorTick(): Promise<void> {
    if (this.stopRequested || this.mockMode) {
      return;
    }

    try {
      const connected = this.modbusService.isConnected();

      if (!connected) {
        if (!this.linkDownActive) {
          this.linkDownActive = true;
          await this.markDevicesCommunicationError(await this.getRegisteredDevicesWithCache());
          notifyModbusLink(true);
          this.logger?.warn('Modbus 링크 down — 등록 유닛 통신 이상(2) 반영');
        }
        return;
      }

      if (this.linkDownActive) {
        const probeOk = await this.probeModbusLink();
        if (probeOk) {
          this.linkDownActive = false;
          await this.markDevicesCommunicationSuccess(await this.getRegisteredDevicesWithCache());
          notifyModbusLink(false);
          this.logger?.info('Modbus 링크 복구 probe 성공 — 등록 유닛 통신 정상(0) 반영');
        }
      }
    } catch (error) {
      this.logger?.warn(
        `링크 감시 tick 실패: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }

  /** 슬레이브 1 · HVAC 현재온도 레지스터 1워드 probe */
  private async probeModbusLink(): Promise<boolean> {
    if (this.mockMode) {
      return true;
    }

    const probeAddress = this.addressMapping?.hvac?.curTemp ?? 120;
    try {
      ...
      return result.success === true && Array.isArray(result.data) && result.data.length > 0;
    } catch {
      return false;
    }
  }

  private async applyUnitCommunicationStatusFromPolling(
    deviceId: string,
    unitId: string,
    deviceType: string,
  ...
  private async markDevicesCommunicationError(
    devices: Array<{ deviceId: string; unitId: string; deviceType?: string }>,
  ): Promise<void> {
    for (const device of devices) {
      await this.handleCommunicationErrorForDevice(device.deviceId, device.unitId, { ws: false });
    }
  }

  private async markDevicesCommunicationSuccess(
    devices: Array<{ deviceId: string; unitId: string; deviceType?: string }>,
  ): Promise<void> {
    for (const device of devices) {
      await this.handleCommunicationSuccess(device.deviceId, device.unitId, { ws: false });
    }
  }
   ...
   * 🆕 개별 장비 통신 에러 처리
   */
  private async handleCommunicationErrorForDevice(
    deviceId: string,
    unitId: string,
    options?: { ws?: boolean },
  ): Promise<void> {
    try {
      ...
      this.logger?.warn(`통신 에러 처리 - ${deviceId}/${unitId}`);
      ...
      if (options?.ws !== false) {
        notifyUnitDdcCommunication(deviceId, unitId, true);
      }
    } catch (error) {
      this.logger?.error(`개별 장비 통신 에러 처리 실패: ${error}`);
    }
  }
   ...
   * 🆕 통신 성공 처리 (개별 장비)
   */
  private async handleCommunicationSuccess(
    deviceId: string,
    unitId: string,
    options?: { ws?: boolean },
  ): Promise<void> {
    try {
      ...
      this.logger?.debug(`통신 성공 처리 - ${deviceId}/${unitId}`);
      ...
      if (options?.ws !== false) {
        notifyUnitDdcCommunication(deviceId, unitId, false);
      }
    } catch (error) {
      this.logger?.error(`통신 성공 처리 실패: ${error}`);
    }
  }
}
