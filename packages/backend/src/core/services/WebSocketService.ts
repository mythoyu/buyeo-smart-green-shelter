...
export class WebSocketService implements IWebSocketService {
  ...
  broadcastPollingRecoveryPrompt(promptId: string, expiresAt: string, autoDismissSec: number): void {
    const message: PollingRecoveryPromptMessage = {
      ...
      data: {
        promptId,
        autoDismissSec: autoDismissSec || POLLING_RECOVERY_AUTO_DISMISS_SEC,
        expiresAt,
      },
    };
  ...
  broadcastPollingRecoveryResolved(
    ...
    const message: PollingRecoveryResolvedMessage = {
      type: 'polling-recovery-resolved',
      level: 'info',
      service: 'system',
      message: '폴링 자동 복구 제안 종료',
      timestamp: nowKstFormatted(),
      data: { promptId, reason },
    };
    this.broadcast(message);
  }
