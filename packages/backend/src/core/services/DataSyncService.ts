  ...
  private async classifyBatchDataByCollection(batchResults: any[]): Promise<{
    ...
    for (const item of batchResults) {
      try {
        ...
        if (dataUpdate) {
          // CommandResultHandler가 필드별 $set으로 이미 반영한 경우 data는 비어 있음 — 전체 data 덮어쓰기 금지
          const hasAggregatedPayload =
            dataUpdate.data != null && typeof dataUpdate.data === 'object' && Object.keys(dataUpdate.data).length > 0;
          if (hasAggregatedPayload) {
            dataCollection.push({
              deviceId: dataUpdate.deviceId,
              unitId: dataUpdate.unitId,
