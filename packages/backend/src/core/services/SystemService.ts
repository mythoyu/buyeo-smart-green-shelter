...
import {
  DEFAULT_POLLING_CYCLE_GAP_MS,
  isAllowedPollingCycleGap,
  normalizePollingCycleGap,
...
export class SystemService implements ISystemService {
  ...
  async updatePollingState(pollingEnabled: boolean): Promise<SystemSettings | null> {
    try {
      ...
      const currentRuntime = currentSettings?.runtime || {
        ...
        pollingInterval: DEFAULT_POLLING_CYCLE_GAP_MS,
      ...
      const cleanRuntime: NonNullable<SystemSettings['runtime']> = {
        ...currentRuntime,
        pollingEnabled,
        pollingInterval: normalizePollingCycleGap(currentRuntime.pollingInterval),
        applyInProgress: currentRuntime.applyInProgress ?? false,
        peopleCounterEnabled: currentRuntime.peopleCounterEnabled ?? true,
      };

      if (!pollingEnabled) {
        delete cleanRuntime.recoveryPrompt;
      }
  ...
  async getPollingState(initializeIfMissing = false): Promise<{
    ...
    try {
      ...
      if (!settings?.runtime) {
        ...
        if (initializeIfMissing) {
          ...
          return {
            ...
            ...(updated.runtime.recoveryPrompt ? { recoveryPrompt: updated.runtime.recoveryPrompt } : {}),
          };
        }
      ...
      return {
        ...
        ...(settings.runtime.recoveryPrompt ? { recoveryPrompt: settings.runtime.recoveryPrompt } : {}),
      };
    } catch (error) {
  ...
  async setRecoveryPrompt(prompt: PollingRecoveryPrompt): Promise<SystemSettings | null> {
    const currentSettings = await this.getSettings();
    const currentRuntime = currentSettings?.runtime || {
      pollingEnabled: false,
      pollingInterval: DEFAULT_POLLING_CYCLE_GAP_MS,
    ...
    return this.systemRepository.updateSettings({
      runtime: {
        ...currentRuntime,
  ...
  async updateRecoveryPromptStatus(
    promptId: string,
    status: PollingRecoveryPrompt['status'],
  ): Promise<SystemSettings | null> {
    const currentSettings = await this.getSettings();
    const prompt = currentSettings?.runtime?.recoveryPrompt;
    ...
    if (status === 'pending') {
      return this.systemRepository.updateSettings({
        runtime: { ...currentRuntime, recoveryPrompt: { ...prompt, status } },
      });
    }

    const { recoveryPrompt: _removed, ...runtimeRest } = currentRuntime;
    return this.systemRepository.updateSettings({
      runtime: runtimeRest as NonNullable<SystemSettings['runtime']>,
    });
  }

  async clearRecoveryPrompt(): Promise<SystemSettings | null> {
    const currentSettings = await this.getSettings();
    if (!currentSettings?.runtime?.recoveryPrompt) {
      return currentSettings;
    }

    const { recoveryPrompt: _removed, ...runtimeRest } = currentSettings.runtime;
    return this.systemRepository.updateSettings({
      runtime: runtimeRest as NonNullable<SystemSettings['runtime']>,
    });
  }
  ...
  async updatePollingInterval(pollingInterval: number): Promise<SystemSettings | null> {
    try {
      if (!isAllowedPollingCycleGap(pollingInterval)) {
        throw new Error('허용되지 않는 폴링 사이클 대기 간격입니다.');
      }
      const gapMs = normalizePollingCycleGap(pollingInterval);
      this.logger?.info(`폴링 사이클 대기 간격 업데이트: ${gapMs}ms`);
      ...
      const updated = await this.systemRepository.updateSettings({
        runtime: {
          ...currentRuntime,
          pollingInterval: gapMs,
        },
      });

      if (updated) {
        this.logger?.info(`폴링 사이클 대기 간격 업데이트 완료: ${gapMs}ms`);
      }

      const pollingService = ServiceContainer.getInstance().getUnifiedModbusPollerService();
      if (pollingService) {
        await pollingService.schedulePollingIntervalChange(gapMs);
        this.logger?.info(`폴링 서비스에 간격 변경 알림 완료: ${gapMs}ms`);
      }

      this.webSocketService?.broadcastLog('info', 'system', `폴링 사이클 대기 간격이 ${gapMs}ms로 업데이트되었습니다.`);
  ...
  async setAllUnitsToScheduleMode(): Promise<{ unitsUpdated: number }> {
    try {
      this.logger?.info('자동모드 활성화: CLIENT_PORT_MAPPINGS 기반으로 모든 장비의 AUTO를 schedule로 변경');

      const clientId = await this.getActiveClientId();
      const { successCount, failureCount } = await this.executeAllAutoCommands(clientId, 1, '자동');

      await this.updateAllUnitsAutoField(true);

      this.logger?.info(`자동모드 활성화 완료: ${successCount}개 성공, ${failureCount}개 실패`);

      this.webSocketService?.broadcastLog(
        'info',
        'system',
        `자동모드 활성화 완료: ${successCount}개 장비의 AUTO가 schedule로 변경되었습니다.`,
      );

      return { unitsUpdated: successCount };
    } catch (error) {
      this.logger?.error('자동모드 활성화 중 오류 발생');
  ...
  async setAllUnitsToManualMode(): Promise<{ unitsUpdated: number }> {
    try {
      this.logger?.info('수동모드 활성화: CLIENT_PORT_MAPPINGS 기반으로 모든 장비의 AUTO를 manual로 변경');

      const clientId = await this.getActiveClientId();
      const { successCount, failureCount } = await this.executeAllAutoCommands(clientId, 0, '수동');

      await this.updateAllUnitsAutoField(false);

      this.logger?.info(`수동모드 활성화 완료: ${successCount}개 성공, ${failureCount}개 실패`);

      this.webSocketService?.broadcastLog(
        'info',
        'system',
        `수동모드 활성화 완료: ${successCount}개 장비의 AUTO가 manual로 변경되었습니다.`,
      );

      return { unitsUpdated: successCount };
    } catch (error) {
      this.logger?.error('수동모드 활성화 중 오류 발생');
    ...
    try {
      ...
      if (seasonal) {
        return {
          ...
          september: seasonal.september ?? 1,
          october: seasonal.october ?? 1,
  ...
  private async getClientPortMapping(clientId: string): Promise<any> {
    try {
      // TS 기본값 + DB 오버라이드 병합 매핑 사용
      return getEffectiveClientMapping(clientId);
    } catch (error) {
  ...
  /** 스냅샷 시스템 설정 적용 */
  async applySnapshotSettings(snapshotData: Record<string, unknown>, appliedBy?: string): Promise<void> {
    this.logger?.info(`스냅샷 시스템 설정 적용: ${appliedBy ?? 'unknown'}`);
    await this.updateSettings(snapshotData as Record<string, unknown>);
  }
}
