  ...
  async getJson<T>(
    ...
    while (attempt <= retries) {
      ...
      try {
        ...
        logDebug(`[ExternalHttpClient] GET ${url} -> ${res.status} (${latency}ms), Content-Type: ${contentType}`);

        if (!res.ok) {
          logDebug(`[ExternalHttpClient] HTTP 에러 응답: ${res.status} ${res.statusText}`);
        ...
        logDebug(`[ExternalHttpClient] 시도 ${attempt + 1} 실패: ${errorMessage}`);
    ...
    logDebug(`[ExternalHttpClient] GET ${url} failed after ${retries + 1} attempts: ${finalErrorMessage}`);
