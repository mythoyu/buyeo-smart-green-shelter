...
const DatabaseStatusCard: React.FC<DatabaseStatusCardProps> = ({ data, onViewData }) => {
  if (!data) {
    return (
      <MonitoringCard icon={Database} title='데이터베이스'>
  ...
  const footer =
    data.error || data.status !== 'connected' ? (
      <>
        {data.error && (
  ...
  return (
    <MonitoringCard icon={Database} title='데이터베이스' headerRight={statusBadge} footer={footer}>
      {/* 연결 상태 상세 */}
