...
export const SystemSettingsTab: React.FC<SystemSettingsTabProps> = ({
  ...
  const ddcTimeRead = useDdcTimeRead();
  ...
  const handleReadDdcTime = async () => {
    try {
      const result = await ddcTimeRead.mutateAsync();
      if (result.success && result.data) {
        setSystemSettings(prev => ({
          ...prev,
          ddcTime: {
            ...
            second: result.data!.second,
          },
        }));
        if (result.data.dow !== undefined) {
          setDdcDow(result.data.dow);
        }
      }
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : '알 수 없는 오류';
      toast.error(`DDC 시간 읽기 실패: ${message}`, { id: 'hw-system-ddc-read-error' });
      onError?.({
        type: 'hardware_error',
        message,
        details: 'DDC 시간 읽기',
  ...
  const readAllRef = useRef(async () => {
    await handleReadSeasonal();
    await handleReadDdcTime();
  });
  readAllRef.current = async () => {
    await handleReadSeasonal();
    await handleReadDdcTime();
  };
  ...
  const isLoading =
    sendCommand.isPending || seasonalRead.isPending || seasonalSet.isPending || ddcTimeRead.isPending;
    ...
    <div className='grid grid-cols-1 md:grid-cols-2 gap-6 relative'>
      ...
      <Card>
        <CardHeader className='flex flex-row items-center justify-between'>
          ...
          {!pollingStatus?.pollingEnabled && (
            <Button
              onClick={() => void handleReadDdcTime()}
              disabled={isLoading}
              variant='outline'
              size='sm'
              className='flex items-center gap-2'
            >
              <RefreshCw className={`h-4 w-4 ${ddcTimeRead.isPending ? 'animate-spin' : ''}`} />
              DDC 시간 읽기
            </Button>
          )}
        </CardHeader>
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  if (failed.length > 0) toast.error(`일부 실패: ${failed.join(', ')}`);
                } finally {
