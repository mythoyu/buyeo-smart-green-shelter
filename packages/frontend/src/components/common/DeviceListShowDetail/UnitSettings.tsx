...
export const UnitSettings: React.FC<UnitSettingsProps> = ({
  ...
  const getAvailableUnitsForField = useCallback(
    ...
    [devices, device.id, device.type, deviceSpecs, unit]
  );
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-3'>
        {timeCommands.map((cmd, index) => {
          ...
          return (
            ...
            >
              <Label className='text-xs font-medium flex items-center justify-between text-gray-700 dark:text-gray-300'>
              ...
              <div className='pointer-events-auto'>
                <TimeSelect
                  ...
                  onChange={(val: string) => onFormChange(cmd.key, val)}
                  interval={1}
                  className='w-full'
                />
              </div>
      ...
      <div className='flex gap-3 mt-6 pt-4 border-t border-gray-100 dark:border-gray-700'>
        ...
        {!isPeopleCounter && (
          <Button
            ...
            disabled={Boolean(bulkCommandsMutation?.isPending)}
