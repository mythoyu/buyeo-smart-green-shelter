...
export const DeviceCard: React.FC<DeviceCardProps> = ({
  ...
  const resolveValue = (key: string) => {
    ...
    return resolveUnitFieldValue(key, primaryUnitData, primaryUnitForm, cmd?.defaultValue);
  };
  ...
  const { powerValue: rawPowerValue, autoValue: rawAutoValue } = resolvePowerAutoValues(
    primaryUnitData,
    primaryUnitForm
  );
  const powerValue = Boolean(rawPowerValue);
  const autoValue = Boolean(rawAutoValue);
  const isPanelProcessing = commandManager.commandStatus === 'loading';
  const isPowerDisabled = isPanelProcessing || autoValue;

  const handleTogglePower = async () => {
    if (!primaryUnit || isPowerDisabled) return;
              ...
              {hasPowerCommand && (
                <LabeledStatusButton
                  ...
                  className={`h-8 px-3 py-1 text-[11px] font-semibold transition-colors ${
                    isPowerDisabled
                      ? 'opacity-50 cursor-not-allowed bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500 border-gray-300 dark:border-gray-600'
                      : powerValue
                  ...
                  {...(!powerValue && !isPowerDisabled && { statusBadgeClassName: 'bg-slate-600 text-white dark:bg-slate-500 border-0' })}
                >
                  전원
                </LabeledStatusButton>
              )}
