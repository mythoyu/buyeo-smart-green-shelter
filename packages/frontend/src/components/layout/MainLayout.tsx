...
import {
  type PollingState,
  useGetPollingState,
  useRespondToPollingRecovery,
  useUpdatePollingState,
} from '../../api/queries/polling';
import { useGetExhibitionMode } from '../../api/queries/system';
...
import { PollingRecoveryDialog, PollingRecoveryPromptData } from '../common/PollingRecoveryDialog';
...
const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
  ...
  // 전시장 모드 (ON이면 화면에서 에러 0처럼 보이도록 덮어쓰기)
  const { data: exhibitionModeData } = useGetExhibitionMode();
  const exhibitionMode = !!exhibitionModeData?.exhibitionMode;

  // ✅ 에러 개수 계산
  const errorCount = exhibitionMode
    ? 0
  ...
  const openRecoveryDialog = useCallback(
    (data: PollingRecoveryPromptData) => {
      if (isDashboardRoute) {
        return;
      }
      setRecoveryDialog({ isOpen: true, prompt: data });
    },
    [isDashboardRoute],
  );

  const closeRecoveryDialog = useCallback(() => {
    setRecoveryDialog({ isOpen: false, prompt: null });
  }, []);

  const goToDashboardIfNeeded = useCallback(() => {
    if (!isDashboardRoute) {
      navigate('/dashboard', { replace: true });
    }
  }, [isDashboardRoute, navigate]);
  const goToDashboardIfNeededRef = useRef(goToDashboardIfNeeded);
  goToDashboardIfNeededRef.current = goToDashboardIfNeeded;

  const handleRecoveryAccept = useCallback(async () => {
    const promptId = recoveryDialogRef.current.prompt?.promptId;
    if (!promptId || recoveryActionInFlightRef.current) {
      return;
    }
    recoveryActionInFlightRef.current = true;
    try {
      await respondToRecoveryMutation.mutateAsync({ promptId, action: 'accept' });
      closeRecoveryDialog();
      goToDashboardIfNeeded();
    } catch (error) {
      console.error('[MainLayout] 폴링 복구 accept 실패:', error);
      await queryClient.refetchQueries({ queryKey: ['polling', 'state'] });
      const latest = queryClient.getQueryData<PollingState>(['polling', 'state']);
      if (latest?.pollingEnabled) {
        closeRecoveryDialog();
        goToDashboardIfNeeded();
        toast.info('폴링이 이미 복구되었습니다.');
        return;
      }
      const message =
        (error as { response?: { data?: { message?: string } } })?.response?.data?.message ??
        '폴링 복구에 실패했습니다. 버튼을 다시 눌러 주세요.';
      toast.error(message, { id: 'polling-recovery-accept-error' });
    } finally {
      recoveryActionInFlightRef.current = false;
    }
  }, [respondToRecoveryMutation, closeRecoveryDialog, goToDashboardIfNeeded, queryClient]);
  ...
  const handleRecoveryDismiss = useCallback(async () => {
    ...
    recoveryActionInFlightRef.current = true;
    try {
      await respondToRecoveryMutation.mutateAsync({ promptId, action: 'dismiss' });
      closeRecoveryDialog();
    } catch (error) {
      console.error('[MainLayout] 폴링 복구 dismiss 실패:', error);
      toast.error('폴링 중지 유지 처리에 실패했습니다.', { id: 'polling-recovery-dismiss-error' });
    } finally {
      recoveryActionInFlightRef.current = false;
    }
  }, [respondToRecoveryMutation, closeRecoveryDialog]);

  useEffect(() => {
    const pending = pollingState?.recoveryPrompt;
    if (!pending || pending.status !== 'pending' || isDashboardRoute) {
      return;
    }
    openRecoveryDialog({
      promptId: pending.promptId,
      autoDismissSec: pollingState?.autoDismissSec ?? 30,
      expiresAt: pending.expiresAt,
    });
  }, [
    pollingState?.recoveryPrompt?.promptId,
    pollingState?.recoveryPrompt?.status,
    pollingState?.autoDismissSec,
    isDashboardRoute,
    openRecoveryDialog,
  ]);

  useEffect(() => {
    if (!recoveryDialog.isOpen || !recoveryDialog.prompt?.promptId) {
      return;
    }

    const openId = recoveryDialog.prompt.promptId;
    const pending = pollingState?.recoveryPrompt;
    const stillPending = pending?.status === 'pending' && pending.promptId === openId;
    ...
    pollingState?.recoveryPrompt,
    pollingState?.pollingEnabled,
    recoveryDialog.isOpen,
    recoveryDialog.prompt?.promptId,
    closeRecoveryDialog,
    goToDashboardIfNeeded,
  ]);
  ...
  useWebSocket({
    ...
    onPollingRecoveryPrompt: message => {
      openRecoveryDialog({
        promptId: message.data.promptId,
        autoDismissSec: message.data.autoDismissSec,
        expiresAt: message.data.expiresAt,
      });
    },
    onPollingRecoveryResolved: message => {
      const { promptId, reason } = message.data;
      setRecoveryDialog(prev => {
        if (prev.prompt?.promptId !== promptId) {
          return prev;
        }
        return { isOpen: false, prompt: null };
      });
      if (reason === 'accepted' || reason === 'expired') {
        goToDashboardIfNeededRef.current();
      }
    },
    onConnect: () => {
  ...
  const navigation = [
    ...
    ...(user?.role === 'superuser' || user?.role === 'engineer'
      ? [
          ...
          { name: '접점 포트 설정', label: '접점\n포트', href: '/port-assignment', icon: Cable },
          { name: '매뉴얼', label: '매뉴\n얼', href: '/manual', icon: BookOpen },
        ]
      : []),
  ...
  return (
    <div className='min-h-screen bg-background'>
      ...
      {/* ✅ 에러 패널 렌더링 */}
      <ErrorPanel
        isOpen={!exhibitionMode && errorPanelOpen}
        onClose={() => setErrorPanelOpen(false)}
        errors={exhibitionMode ? undefined : clientErrorsData}
      />
      ...
      <PollingRecoveryDialog
        isOpen={recoveryDialog.isOpen}
        prompt={recoveryDialog.prompt}
        onAcceptDashboard={() => void handleRecoveryAccept()}
        onDismiss={() => void handleRecoveryDismiss()}
        onClose={closeRecoveryDialog}
        onAutoExpire={() => void handleRecoveryAccept()}
      />
    </div>
  );
};
