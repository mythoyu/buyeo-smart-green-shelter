import { Clock, Sun, Cpu, RefreshCcw, Users, Activity, Settings, Crown } from 'lucide-react';
...
import {
  ...
  useRefreshDdcTime,
  useGetSystemTime,
  useGetExhibitionMode,
  useSetExhibitionMode,
...
import {
  ALLOWED_POLLING_CYCLE_GAP_MS,
  DEFAULT_POLLING_CYCLE_GAP_MS,
  formatPollingCycleGapLabel,
...
const SystemSettingsPage: React.FC = () => {
  ...
  const setExhibitionModeMutation = useSetExhibitionMode();
  ...
  // 폴링 간격 상태
  const [pollingIntervalInput, setPollingIntervalInput] = useState<number>(DEFAULT_POLLING_CYCLE_GAP_MS);
  ...
  const [exhibitionModeInput, setExhibitionModeInput] = useState(false);
  ...
  // 🔄 폴링 간격 초기 로드
  useEffect(() => {
    if (pollingIntervalData?.data?.pollingInterval != null) {
      setPollingIntervalInput(normalizePollingCycleGap(pollingIntervalData.data.pollingInterval));
    }
  }, [pollingIntervalData]);

  useEffect(() => {
    if (exhibitionModeData?.exhibitionMode !== undefined) {
      setExhibitionModeInput(!!exhibitionModeData.exhibitionMode);
    }
  }, [exhibitionModeData?.exhibitionMode]);
  ...
  const handlePollingIntervalApply = async () => {
    try {
      await setPollingIntervalMutation.mutateAsync(pollingIntervalInput);
    } catch (error: any) {
      const message =
  ...
  const handleExhibitionModeApply = () => {
    setExhibitionModeMutation.mutate(exhibitionModeInput, {
      onSuccess: () => {},
      onError: (error: any) => {
        const message =
          error?.response?.data?.message || error?.response?.data?.error || '전시장 모드 설정에 실패했습니다.';
        toast.error(message, { id: 'exhibition-mode-error' });
      },
    });
  };
      ...
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 items-stretch'>
          ...
          >
            <div className='p-3 bg-muted rounded-lg space-y-2'>
              <div className='flex justify-between items-center'>
                <span className='text-sm font-medium'>서버 현재 시간 (KST, ISO)</span>
        ...
        >
          ...
          >
            ...
            >
              {ALLOWED_POLLING_CYCLE_GAP_MS.map(ms => (
                <SelectItem key={ms} value={String(ms)}>
                  {formatPollingCycleGapLabel(ms)}
                </SelectItem>
              ))}
            </SelectWithLabel>
          </SettingsCard>
        ...
        {/* 전시장 모드 카드 */}
        {visibleCardIds.includes('settings-exhibition') && (
        <div
          id='settings-exhibition'
          className='h-full'
          style={{
        ...
        >
          <SettingsCard
            icon={Crown}
            title='전시장 모드'
            description='하드웨어 연결 없이 화면을 정상처럼 표시합니다.'
            onApply={handleExhibitionModeApply}
            applyDisabled={
              setExhibitionModeMutation.isPending || exhibitionModeInput === !!exhibitionModeData?.exhibitionMode
            }
            currentSettings={null}
            isLoading={setExhibitionModeMutation.isPending}
          >
            <div className='flex items-center justify-between p-3 bg-muted rounded-lg'>
              <div className='flex items-center gap-2'>
                <span className='text-sm font-medium'>전시장 모드 활성화</span>
                <Badge variant={exhibitionModeInput ? 'default' : 'secondary'} className='text-xs'>
                  {exhibitionModeInput ? 'ON' : 'OFF'}
                </Badge>
              </div>
              <OnOffToggleButton
                checked={exhibitionModeInput}
                onChange={setExhibitionModeInput}
                labelOn='ON'
                labelOff='OFF'
              />
            </div>
