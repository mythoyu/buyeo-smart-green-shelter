import { AlertCircle, Zap, Radio, Thermometer, Gauge, Settings, Users, Flame } from 'lucide-react';
...
const CONSTANTS = {
  ...
  USAGE_GUIDE: {
    ...
    ITEMS: [
      ...
      { label: '온열벤치', desc: '온열벤치 설정 온도·편차·기동 체크시간 직접 제어' },
...
const HardwareControlPage: React.FC = React.memo(() => {
  ...
  // 폴링 상태 확인 (메모이제이션)
  const isPollingBlocking = useMemo(
    () => pollingStatus?.modbusPollingActive ?? pollingStatus?.pollingEnabled ?? false,
    [pollingStatus?.modbusPollingActive, pollingStatus?.pollingEnabled]
  );
  ...
  // 폴링 중지 후에만 탭 자동 읽기·제어 허용 (다이얼로그만 닫고 폴링 ON이면 불가)
  const operateEnabled = useMemo(
    () => !pollingLoading && !isPollingBlocking && !pollingDialog.isOpen,
    [pollingLoading, isPollingBlocking, pollingDialog.isOpen]
  );
  ...
  return (
    <div className='w-full space-y-6'>
      {/* 폴링 활성화 경고 */}
      {/* {isPollingBlocking && <PollingWarningAlert />} */}

      {/* 하드웨어 제어 탭 (사이드바에서 선택, 탭 리스트 숨김) */}
      <HardwareControlTabs
        disabled={isPollingBlocking}
        operateEnabled={operateEnabled}
        onError={handleError}
