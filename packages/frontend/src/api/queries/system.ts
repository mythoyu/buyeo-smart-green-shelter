...
import type { SystemTimeData } from './system-time';
...
// 서버 현재 시간 조회
const getSystemTime = async (): Promise<{ success: boolean; message: string; data: SystemTimeData }> => {
...
// ==================== 🏛 전시장 모드 API ====================
const getExhibitionMode = async (): Promise<{ exhibitionMode: boolean }> => {
  return internalApi.get('/system/exhibition-mode').then(res => res.data.data);
};

const setExhibitionMode = async (exhibitionMode: boolean): Promise<{ exhibitionMode: boolean }> => {
  return internalApi.post('/system/exhibition-mode', { exhibitionMode }).then(res => res.data.data);
};

export const useGetExhibitionMode = () =>
  useQuery({
    queryKey: ['system', 'exhibition-mode'],
    queryFn: getExhibitionMode,
  });

export const useSetExhibitionMode = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: setExhibitionMode,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['system', 'exhibition-mode'] });
    },
  });
};
