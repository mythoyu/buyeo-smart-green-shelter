 ...
 * 하드웨어 직접 제어 명령 전송 훅
 */
export interface HardwareBulkCommandRequest {
  command: string;
  value: boolean | number;
  ports?: string[];
}

export interface HardwareBulkCommandResponse {
  success: boolean;
  message: string;
  data?: {
    commandId: string;
...
const sendBulkHardwareCommand = async (params: HardwareBulkCommandRequest): Promise<HardwareBulkCommandResponse> => {
  const { data } = await internalApi.post<HardwareBulkCommandResponse>('/hardware/bulk-command', params);
  return data;
};

export const useSendBulkHardwareCommand = () => {
  return useMutation({
    mutationFn: sendBulkHardwareCommand,
    onError: (error: Error) => {
      console.error('[HardwareBulkCommand] 일괄 제어 실패:', error.message);
    },
  });
};
...
type BenchReadResponse = { success: boolean; message: string; data?: BenchData };

type BenchSetRequest = {
  cont_temp?: number;
  cont_temp_2?: number;
  temp_offset?: number;
...
const benchRead = async (): Promise<BenchReadResponse> => {
  const { data } = await internalApi.post<BenchReadResponse>('/hardware/system/bench', { action: 'read' });
  return data;
};

const benchSet = async (params: BenchSetRequest): Promise<boolean> => {
  const { data } = await internalApi.post('/hardware/system/bench', { action: 'set', ...params });
  return !!(data as { success?: boolean })?.success;
};

export const useBenchRead = () => useMutation({ mutationFn: benchRead });
export const useBenchSet = () => useMutation({ mutationFn: benchSet });
