...
import { DeviceListShowDetailProps, SelectedUnit, UnitForm, Device } from './types';
import { getInitialFormValues } from './utils';

const DeviceListShowDetail: React.FC<DeviceListShowDetailProps> = ({ devices, deviceSpecs, deviceStyles }) => {
  ...
  console.log('DeviceListShowDetail Debug:', {
    ...
    // 디바이스 이름 정보 추가
    deviceNames: displayDevices?.map((d: Device) => ({
      id: d.id,
      name: d.name,
      deviceId: d.deviceId,
    })),
    ...
    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4'>
      {displayDevices.map((device: Device, deviceIndex: number) => (
        <DeviceCard
          ...
          getUnitForm={() => (selectedUnit ? getUnitForm(selectedUnit.device.id, selectedUnit.unit.id) : {})}
