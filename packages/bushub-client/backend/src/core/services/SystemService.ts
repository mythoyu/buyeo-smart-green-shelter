...
export class SystemService implements ISystemService {
  ...
  async setAllUnitsToScheduleMode(): Promise<{ unitsUpdated: number }> {
    try {
      ...
      // 2. data 컬렉션의 auto 필드를 true로 변경
      await this.updateAllUnitsAutoField(true);

      // 3. 시스템 모드를 'auto'로 변경하여 DB에 저장 (제거됨)
      // await this.updateSystemMode('auto');
  ...
  async setAllUnitsToManualMode(): Promise<{ unitsUpdated: number }> {
    try {
      ...
      // 2. data 컬렉션의 auto 필드를 false로 변경
      await this.updateAllUnitsAutoField(false);

      // 3. 시스템 모드를 'manual'로 변경하여 DB에 저장 (제거됨)
      // await this.updateSystemMode('manual');
