#!/bin/bash
# Modbus(DDC)용 USB-시리얼 포트를 udev KERNELS 조건으로 식별해
# /dev/bushub-controller 심볼릭 링크 생성 규칙 1줄을 생성합니다.
#
# 사용법:
#  1) Modbus용 USB 어댑터만 1개 꽂은 상태에서 이 스크립트를 실행
#  2) 출력된 tty 디바이스를 선택
#  3) KERNELS 후보 중 적절한 값을 선택
#  4) fragment 파일이 /tmp/bushub-udev-rules 에 생성됨
#
# fragment 생성 후:
#  ./03-install-bushub-usb-serial.sh 로 /etc/udev/rules.d/에 설치
#
...
FRAGMENT_FILE="$TMP_DIR/99-bushub-serial-controller.fragment"

SYMLINK_NAME="bushub-controller"
...
if [ "$LIST_JSON" -eq 1 ]; then
  if [ ${#TTY_LIST[@]} -eq 0 ]; then
    ...
    echo "   Modbus용 USB 어댑터를 먼저 꽂고 다시 실행하세요." >&2
    exit 1
  fi
  emit_tty_list_json
  exit 0
...
if [ ${#TTY_LIST[@]} -eq 0 ]; then
  ...
  echo "   Modbus용 USB 어댑터를 먼저 꽂고 다시 실행하세요."
  exit 1
fi
...
if [[ "$(basename "$DEVICE")" =~ ^ttyUSB ]]; then
  ...
  # 대개 ttyUSB/ttyACM 만 쓰겠지만, 안전하게 처리
...
# (선택) vendor/product/serial이 있으면 추가 매칭으로 오매칭을 줄임
...
echo "✅ 컨트롤러 fragment 생성 완료: $FRAGMENT_FILE"
echo "   다음 단계:"
echo "   1) PeopleCounter용도 동일하게 02-probe-pc-bushub-usb-serial.sh 실행"
echo "   2) sudo ./03-install-bushub-usb-serial.sh"
echo "   (또는 ./00-wizard-bushub-usb-serial.sh 로 한 번에 진행)"
