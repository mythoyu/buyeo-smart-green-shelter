#!/usr/bin/env bash
# udev 규칙을 설치하고 /dev/bushub-controller, /dev/bushub-people-counter-1..N 를 생성합니다.
#
# 이 스크립트는 반드시 백업을 수행합니다.
#
# 옵션:
#   --strict  컨트롤러 링크 + (count>=1이면) people-counter-1..N 링크가 모두 없으면 exit 1 (마법사·CI용)
#
set -euo pipefail
...
if [ "$PEOPLE_COUNTER_COUNT" -ge 1 ]; then
  for i in $(seq 1 "$PEOPLE_COUNTER_COUNT"); do
    ...
    if [ ! -s "$pc_frag" ]; then
      ...
      echo "   ./02-probe-pc-bushub-usb-serial.sh --index $i 부터 실행하세요."
      exit 1
    fi
  done
fi

echo "✅ fragment 존재 확인 완료 (PEOPLE_COUNTER_COUNT=$PEOPLE_COUNTER_COUNT)"
