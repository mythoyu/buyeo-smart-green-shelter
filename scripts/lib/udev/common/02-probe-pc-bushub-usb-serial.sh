...
# /dev/bushub-people-counter-N 심볼릭 링크 생성 규칙 1줄(fragment)을 생성합니다.
#
# usb485 / integrated 공통. 다음 단계는 스택에 맞게 install 스크립트를 실행합니다.
#
set -euo pipefail
...
echo "=== KERNELS 후보 ==="
for j in "${!KERNELS_CANDIDATES[@]}"; do
  n=$((j+1))
  echo "  $n) ${KERNELS_CANDIDATES[$j]}"
done
...
echo "   USB-RS485(Modbus+APC): sudo bash \"$_SCRIPT_ROOT/usb485/03-install-bushub-usb-serial.sh\""
echo "   integrated(Modbus=ttyS0, APC만 USB): sudo bash \"$_SCRIPT_ROOT/integrated/03-install-bushub-serial.sh\""
echo "   (원샷: scripts/lib/udev/00-wizard-bushub-usb-serial.sh)"
