...
**송신(프로젝트 구현)**: ID·명령 사이와 `]` 앞 **공백 없음**. 예) 기본 ID 상태 조회는 ASCII **9바이트** `[0000BTR]` (`5B 30 30 30 30 42 54 52 5D`).
...
**설명**: 현재 상태를 조회합니다. (`xxxx` = 4자리 ID, 예: `[0000BTR]`)
...
> **참고**: 일부 표·자료에 `BTO`로 잘못 표기된 경우가 있으나, 장비 프로토콜 기준 OUT 누적 리셋 명령은 **`BTD`**(3문자)가 맞다.
...
- [APC100_485통신프로토콜.xlsx](./references/APC100_485통신프로토콜.xlsx) — 보탬 제공 원본(`docs/references/`)
